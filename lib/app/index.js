...
App.prototype.addArticle = function(userId, url, action) {
  var id = uuid.v1();
  this.connections.queue.publish(SCRAPE_QUEUE, { id: id, url: url, userId: userId ,action : action});
  return Promise.resolve(id);
};

App.prototype.scrapeArticle = function(userId, id, url, action) {
  logger.log({ type: 'info', msg:+"1" + action});
  return this.scraper(userId, id, url, action);
};
...
App.prototype.handleScrapeJob = function(job, ack) {
  ...
  this
    .scrapeArticle(job.userId, job.id, job.url, job.action)
